...
namespace Game
{
    ...
    public class SpawnHazardEffect : CardEffect
    {
        ...
        protected override void ApplyTo(Actor target, Actor collector)
        {
            ...
            {
                GameObject spawned = Instantiate(_hazardPrefab, PickPosition(collector), Quaternion.identity);

                IHazard hazard = spawned.GetComponent<IHazard>();
                if (hazard != null) hazard.Initialize(collector);
            }
        }
